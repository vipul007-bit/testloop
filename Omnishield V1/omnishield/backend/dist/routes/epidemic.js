...
const SimulateSchema = zod_1.z.object({
    model: zod_1.z.enum(['SIR', 'SEIR']),
    population: zod_1.z.number().min(1000).max(10000000),
    initialInfected: zod_1.z.number().min(1).max(10000),
    r0: zod_1.z.number().min(0.1).max(10),
    gamma: zod_1.z.number().min(0.01).max(0.5), // recovery rate
    sigma: zod_1.z.number().min(0.01).max(0.5).optional(), // incubation rate (SEIR only)
    days: zod_1.z.number().min(1).max(365),
    district: zod_1.z.string().optional(),
});
// SIR model: dS/dt = -β*S*I/N, dI/dt = β*S*I/N - γ*I, dR/dt = γ*I
function runSIR(N, I0, R0, gamma, days) {
    const beta = R0 * gamma;
    const S = [N - I0];
    const I = [I0];
    const R = [0];
...
function runSEIR(N, I0, R0, gamma, sigma, days) {
    const beta = R0 * gamma;
    const S = [N - I0];
    const E = [0];
    const I = [I0];
...
router.post('/simulate', auth_1.requireAuth, async (req, res, next) => {
    try {
...
router.get('/hotspot-propagation', auth_1.requireAuth, async (_req, res, next) => {
    try {
