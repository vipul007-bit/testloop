...
const SYMPTOM_DIAGNOSES: Record<string, { icd: string; name: string; confidence: number }[]> = {
  fever: [
...
const DRUG_INTERACTIONS: Array<{ drug1: string; drug2: string; severity: 'HIGH' | 'MODERATE' | 'LOW'; description: string }> = [
...
router.post('/analyze', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const diagnosisScores: Map<string, { icd: string; name: string; score: number; count: number }> = new Map()
    ...
    for (let i = 0; i < currentMedications.length; i++) {
      for (let j = i + 1; j < currentMedications.length; j++) {
        ...
        for (const interaction of DRUG_INTERACTIONS) {
          if (
            (med1.toLowerCase().includes(interaction.drug1.toLowerCase()) && med2.toLowerCase().includes(interaction.drug2.toLowerCase())) ||
    ...
    const symptomSet = symptoms.map((s: string) => s.toLowerCase()).join(' ')
    ...
    if (medicalHistory.some((h: string) => h.toLowerCase().includes('diabetes'))) {
