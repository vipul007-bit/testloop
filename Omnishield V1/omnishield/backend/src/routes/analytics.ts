...
router.get('/disease-trends', requireAuth, async (_req: Request, res: Response, next: NextFunction) => {
  try {
    let dbData: any[] = []
    try {
      dbData = await knex.raw(`
        SELECT
    ...
    for (const row of dbData) {
      ...
      if (!diseaseMap.has(date)) diseaseMap.set(date, new Map())
...
router.get('/demographic-risk', requireAuth, async (_req: Request, res: Response, next: NextFunction) => {
  try {
...
router.get('/seasonal-forecast', requireAuth, async (_req: Request, res: Response, next: NextFunction) => {
  try {
...
router.get('/capacity-forecast', requireAuth, async (_req: Request, res: Response, next: NextFunction) => {
  try {
    const hospitals = [
...
router.get('/vaccination-impact', requireAuth, async (_req: Request, res: Response, next: NextFunction) => {
  try {
    const data = [
