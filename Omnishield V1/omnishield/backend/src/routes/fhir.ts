...
function fhirMeta(resourceType: string) {
  return {
    versionId: '1',
...
router.post('/Patient', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    try {
      await knex('fhir_resources').insert({ id, resource_type: 'Patient', resource_json: JSON.stringify(resource), created_at: new Date() })
    } catch (_) {}
...
router.get('/Patient/:id', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    let resource = null
    try {
      const row = await knex('fhir_resources').where({ id: req.params.id, resource_type: 'Patient' }).first()
      if (row) resource = typeof row.resource_json === 'string' ? JSON.parse(row.resource_json) : row.resource_json
    } catch (_) {}
    if (!resource) {
      resource = {
...
router.post('/Observation', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const id = uuidv4()
    const resource = {
    ...
    try {
      await knex('fhir_resources').insert({ id, resource_type: 'Observation', resource_json: JSON.stringify(resource), created_at: new Date() })
    } catch (_) {}
    res.status(201).json(resource)
  } catch (err) { next(err) }
})
...
router.post('/DiagnosticReport', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const id = uuidv4()
    const resource = {
    ...
    try {
      await knex('fhir_resources').insert({ id, resource_type: 'DiagnosticReport', resource_json: JSON.stringify(resource), created_at: new Date() })
    } catch (_) {}
    res.status(201).json(resource)
  } catch (err) { next(err) }
})
...
router.post('/MedicationRequest', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const id = uuidv4()
    const resource = {
    ...
    try {
      await knex('fhir_resources').insert({ id, resource_type: 'MedicationRequest', resource_json: JSON.stringify(resource), created_at: new Date() })
    } catch (_) {}
    res.status(201).json(resource)
  } catch (err) { next(err) }
})
...
router.post('/Bundle', requireAuth, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const id = uuidv4()
